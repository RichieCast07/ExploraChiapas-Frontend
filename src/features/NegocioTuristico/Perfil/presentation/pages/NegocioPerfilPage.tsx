...
export function NegocioPerfilPage() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const res = await fetchAuth(`${BASE_URL}/users/profile`);
        const body = await res.json() as { success: boolean; data?: ApiUser };
        if (res.ok && body.data) {
          setNombre(body.data.name);
          setEmail(body.data.email);
          setTelefono(body.data.phone ?? '');
        }
      } catch {
  ...
  const handleGuardar = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetchAuth(`${BASE_URL}/users/profile`, {
        method: 'PATCH',
        body: JSON.stringify({ name: nombre, phone: telefono || null }),
      });
      if (res.ok) {
